...
export class DefaultConfig implements Config {
  ...
  tradeJetGold(dist: number): Gold {
    // Similar to tradeShip but slightly more rewarding for land-to-land routes
    const debuff = this.tradeJetShortRangeDebuff();
    const baseGold =
      90_000 / (1 + Math.exp(-0.03 * (dist - debuff))) + 60 * dist;
    const multiplier = this.goldMultiplier();
    return BigInt(Math.floor(baseGold * multiplier));
  }
  ...
  tradeJetSpawnRate(
    ...
    const decayRate = Math.LN2 / 50;
    const baseSpawnRate = 1 - sigmoid(numTradeJets, decayRate, 200);
    const rejectionModifier = 1 / (tradeJetSpawnRejections + 1);
    return Math.floor((100 * rejectionModifier) / baseSpawnRate);
  }
